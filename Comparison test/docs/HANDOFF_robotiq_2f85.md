...
>
> Current record: `Comparison test/run_log_new.md` (2026-07-30, Day 22) and
...
> `Comparison test/ur5_grasp/tools/check_robotiq_pads.py` first.
...
"2026-07-30 (Day 21...)" in Comparison test/run_log_new.md. Everything below summarises
...
- The arm already has a body called `base_link`, so a gripper prim of the same name is
  ...
  `base_link`.
- Path depth gotcha: scripts under "Comparison test/" are one directory deeper than the
  main folder, which broke a hardcoded "two levels up" IsaacLab lookup (Day 20). The four
  scripts in Comparison test/ur5_grasp/scripts/ use _find_isaaclab_root() instead. Reuse it.
- rsl_rl log paths are CWD-relative, not script-relative — cd into "Comparison test/" first.
...
- Abdur_Rabbi_THESIS/Comparison test/ur5_grasp/ — the working copy. All of this happens here.
