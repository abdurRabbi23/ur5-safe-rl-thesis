...
# gotcha in logbook/09 — it is why runs must be launched from inside "Comparison test/").
...
_PKG_ROOT = os.path.normpath(os.path.join(_HERE, "..", ".."))      # .../Comparison test
...
def main() -> int:
    ...
    try:
        ...
        log('    cd ~/Abdur_Rabbi_THESIS/"Comparison test"')
    ...
    if not runs:
        ...
        log("its logs into IsaacLab/logs/ instead. Re-launch from inside 'Comparison test/'.")
        return 1
