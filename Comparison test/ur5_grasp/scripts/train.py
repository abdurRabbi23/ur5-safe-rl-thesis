...
"""Train the UR5e grasp agent with RSL-RL.

This is Isaac Lab's rsl_rl train.py with two additions (marked TOUHID) so the
external `ur5_grasp` task package is importable and registered:
  1. add the repo root to sys.path      -> `import ur5_grasp.tasks`
  2. add Isaac Lab's rsl_rl scripts dir  -> `import cli_args`

Run from the IsaacLab dir (isaaclab env active):
...
_REPO_ROOT = os.path.normpath(os.path.join(_HERE, "..", ".."))          # Abdur_Rabbi_THESIS
