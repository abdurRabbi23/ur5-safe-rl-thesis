...
\textbf{Citation placeholders.} \todocite{A}, \todocite{B} mark positions where a reference is required
but no sourced entry yet exists in the project bibliography (\texttt{logbook/08\_project\_context.md}).
They must be replaced with real IEEE-numbered entries before submission; they are not
placeholders for text, only for citation numbers. Bracketed numerals \texttt{{[}1{]}}--\texttt{{[}3{]}} refer to the
...
Optimisation baseline. The \texttt{cppo} arm is the constrained PPO-Lagrangian agent \todocite{B} with an
...
\midrule
\endhead
Commit / tag & \texttt{567e4c0}, tag \texttt{matrix-v2}\tabularnewline
Task & \texttt{Isaac-Lift-Cube-UR5e-v0} (frozen weld environment)\tabularnewline
Arms trained & \texttt{ppo}, \texttt{ctrl}, \texttt{cppo}\tabularnewline
Arms not trained in this batch & \texttt{cppo10}, \texttt{sac}\tabularnewline
Training seeds & 1, 2, 3, 4, 5, 50, 51, 52, 53, 54 (n = 10 per arm)\tabularnewline
Training budget & 1500 iterations, 4096 environments, \texttt{rsl\_rl} 3.0.1\tabularnewline
Evaluation seeds & 101, 102, 103 (n = 3, disjoint from training)\tabularnewline
Evaluation protocol & 1000 episodes per checkpoint \(\times\) evaluation seed, 128 environments, deterministic policy, observation corruption off\tabularnewline
\texttt{MANIP\_FLOOR} & 0.06 (recalibrated from 0.045)\tabularnewline
\texttt{JOINT\_LIMIT\_MARGIN} & 0.175 rad (unchanged value, reclassified active)\tabularnewline
\texttt{COLLISION\_Z\_FLOOR} & 0.05 m (unchanged, confirmed inactive)\tabularnewline
\texttt{cost\_limit} & 25.0\tabularnewline
\bottomrule
\end{longtable}
...
\textbf{Table 4.2 --- Training-time performance (tail mean over final 10 \% of iterations, n = 10 seeds).}

\begin{longtable}[]{@{}lll@{}}
\toprule
Arm & Mean reward (\(\pm\) std) & \texttt{viol\_singularity}, soft-margin step fraction (\(\pm\) std)\tabularnewline
\midrule
\endhead
\texttt{ppo} / \texttt{ctrl} (identical) & 133.57 \(\pm\) 1.58 & 0.318 \(\pm\) 0.261\tabularnewline
\texttt{cppo} & 132.65 \(\pm\) 1.80 & 0.261 \(\pm\) 0.124\tabularnewline
\bottomrule
\end{longtable}
...
\textbf{Table 4.3 --- Evaluation-time task performance (frozen deterministic policy; mean over 10 training
seeds, each the mean of 3 evaluation seeds; 30,000 episodes per arm).}

\begin{longtable}[]{@{}lll@{}}
\toprule
Metric & \texttt{ppo} / \texttt{ctrl} & \texttt{cppo}\tabularnewline
\midrule
\endhead
Lift success (\(\ge\) 50 \% of commanded goal height) & 99.86 \% & 99.87 \%\tabularnewline
Goal-reach \textless{} 1 cm & 94.28 \% & 96.49 \%\tabularnewline
Goal-reach \textless{} 2 cm & 99.08 \% & 99.17 \%\tabularnewline
Goal-reach \textless{} 5 cm & 99.81 \% & 99.85 \%\tabularnewline
Goal distance, mean / median / p90 (m) & 0.0060 / 0.0047 / 0.0080 & 0.0054 / 0.0042 / 0.0068\tabularnewline
\bottomrule
\end{longtable}
...
kinematic singularity --- a manipulability measure \todocite{A} below \(10^{-4}\), which is a statement about
...
\textbf{Table 4.4 --- Safety on the frozen policy, pooled over 30,000 evaluation episodes per arm.}
...
\midrule
\endhead
True singularity crossings (w \textless{} \(10^{-4}\)) & 1.343 \% (403 / 30,000 episodes) & 0.250 \% (75 / 30,000 episodes)\tabularnewline
Mean episode-minimum manipulability & 0.05471 & 0.06169\tabularnewline
Worst single-episode manipulability & 0.000001 & 0.000001\tabularnewline
Joint limit touched at all & 5.37 \% of episodes & 0.00 \% of episodes, all 10 seeds\tabularnewline
Collision touched at all & 0.13 \% of episodes & 0.017 \% of episodes\tabularnewline
Episodic safety cost, mean / p90 / max & 47.68 / 180.63 / 343.01 & 18.41 / 73.26 / 224.07\tabularnewline
\bottomrule
\end{longtable}
...
\textbf{Table 4.5 --- Per-seed natural episodic cost (training, tail mean) and the constrained agent's
final multiplier.}

\begin{longtable}[]{@{}lllllllllll@{}}
\toprule
Seed & 1 & 2 & 3 & 4 & 5 & 50 & 51 & 52 & 53 & 54\tabularnewline
\midrule
\endhead
\texttt{ctrl} natural cost & 102.1 & 7.7 & 162.3 & 30.0 & 19.1 & 8.6 & 1.8 & 106.9 & 18.8 & 7.9\tabularnewline
\texttt{cppo} natural cost & 18.0 & 16.6 & 11.9 & 19.7 & 24.1 & 23.9 & 17.0 & 9.5 & 23.5 & 12.0\tabularnewline
\texttt{cppo} \(\lambda\) (final iteration) & 0 & 0 & 0 & 0 & 0.013 & 0.001 & 0 & 0 & 0.154 & 0\tabularnewline
\bottomrule
\end{longtable}
...
\begin{draftonly}
\section*{Provisional references cited in this chapter}\label{provisional-references-cited-in-this-chapter}

To be merged into the thesis-wide IEEE numbered list; numbering here is local to this draft.

{[}1{]} F. Khan \emph{et al.}, ``Reinforcement learning for precision grasping and safety-critical
coordination in a robotic arm,'' \emph{Intelligent Service Robotics}, vol.~19, no. 16, 2026,
doi: 10.1007/s11370-025-00668-0.

{[}2{]} A. A. Shahid \emph{et al.}, ``Learning continuous control actions for robotic grasping with
reinforcement learning,'' \emph{Autonomous Robots}, vol.~46, pp.~483--498, 2022,
doi: 10.1007/s10514-022-10034-z. \emph{(Verify exact title against the PDF before submission --- the
entry in \texttt{08\_project\_context.md} records the venue and DOI but abbreviates the title.)}

{[}3{]} M. M. Khan, ``CSRT tracking and classical image-based visual servoing on a 5-DOF manipulator,''
BSc thesis, Dept. of Mechatronics Engineering, KUET, Khulna, Bangladesh, Dec.~2025. \emph{(Not cited in
this chapter; listed because Chapter 4 of the final book may reference it for baseline
positioning.)}

\textbf{\todocite{A}} --- Yoshikawa's manipulability measure. Required at Section~\ref{sec:r-safety} where w = \(\sqrt{\det(JJ^{\top})}\)
is first used as a safety quantity in this chapter. Not currently in the project bibliography;
must be sourced.

\textbf{\todocite{B}} --- PPO-Lagrangian / constrained policy optimisation. Required at Section~\ref{sec:r-design} where
