...
\begin{draftnote}
MODULE 4 --- reorganise, then extend.

Ported from \texttt{Thesis\_Documentation/Methods\_Chapter\_Layer1.md} (draft 2026-07-19), now a
frozen dated record; this \texttt{.tex} is the source of truth. Numbers verified against
\texttt{ur5\_grasp/} on 2026-07-19.

Under the six-chapter structure this chapter carries everything the earlier seven-chapter plan
was going to split across Chapters 3, 4 and 5, so nothing has to move out --- but it does need
regrouping under the three headings the accepted book uses, and the implementation material is
currently missing:

\begin{itemize}
\item \textbf{3.1 Simulation Platform and Robot Model} --- Isaac Sim / Isaac Lab, the UR5e and
      gripper model, the weld abstraction and why it exists. Mostly present.
\item \textbf{3.2 Software Framework} --- \texttt{ur5\_grasp/} architecture, the cost computer,
      the Lagrangian runner, the training and evaluation pipeline, and the gradient-clip audit
      and its fix. \textbf{Largely absent}; the audit story is currently squeezed into
      Section~\ref{sec:r-validity} of the results chapter and deserves room here.
\item \textbf{3.3 Mathematical Modelling} --- the CMDP, the cost function, the Lagrangian
      formulation, calibration. Present.
\end{itemize}

\textbf{Also open:} this draft predates the Day-23 audit. Check whether it narrates the
withdrawal, since Section~\ref{sec:r-validity} assumes it does.
\end{draftnote}
...
manipulability measure w = \(\sqrt{\det(JJ^{\top})}\) \cite{yoshikawa1985manipulability} is computed from the \(6\times6\) end-effector Jacobian J of the arm,
