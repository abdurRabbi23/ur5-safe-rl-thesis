...
"""Play a trained UR5e grasp checkpoint (RSL-RL).

Isaac Lab's rsl_rl play.py with the same two additions (TOUHID) as train.py so the
external `ur5_grasp` package is importable and its task registered.

Watch the latest trained policy (opens the GUI):

    cd ~/Abdur_Rabbi_THESIS/IsaacLab
    ./isaaclab.sh -p ../ur5_grasp/scripts/play.py \
        --task Isaac-Lift-Cube-UR5e-Play-v0 --num_envs 16
"""
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # extract the neural network module (2.3+ then fallback)
    try:
    ...
    # export policy to onnx/jit (handy for the Layer 3 ROS 2 deployment later)
